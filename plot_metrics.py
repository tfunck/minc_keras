def plot_loss(history_fn,model_fn, report_dir):
    ...
    epoch_num = range(len(history['acc']))
    # train_error = np.subtract(1, np.array(hist.history['acc']))
    # test_error  = np.subtract(1, np.array(hist.history['val_acc']))
    plt.clf()
    plt.subplot(2,1,1)
    plt.plot(epoch_num, np.array(history['acc']), label='Training Accuracy')
    plt.plot(epoch_num, np.array(history['val_acc']), label="Validation Accuracy")
